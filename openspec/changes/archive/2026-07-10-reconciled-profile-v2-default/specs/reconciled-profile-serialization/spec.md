# Spec: reconciled-profile-v2-default
...
This change SHALL be additive-only. `web/`, `api/`, `app.py`, `ui/`, `cli.py`, tests, and `openspec/specs/` SHALL NOT be modified in this spec phase. `core/__init__.py` changes SHALL only add the v2 re-export and SHALL NOT remove legacy exports.

## Design Note

`core.ai_v2.builder.py` does not consume `ReconciledProfile`; it consumes `BenchParams` from comparison rows. Therefore this spec does not require a persistent optional `ReconciledProfile.benches` field. Hazard enrichment is specified through method-level `benches` arguments and can be revisited in `sdd-design` if a stored field is still desired.
