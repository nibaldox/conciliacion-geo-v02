...
> **Source change**: `blast-design-achievement` (archived)
> **Archived on**: 2026-07-12
> **Status**: archived, source of truth

> **Scope override**: temporarily lifts `openspec/config.yaml#off_limits` for `ui/tabs/blast_correlation.py` (blast tab only) per explicit maintainer authorization (one-off, not precedent — same pattern as `2026-07-11-streamlit-audit-remediation`).

## Purpose

Close two closed-loop blast→design KPIs: (a) stemming-crest damage correlation per bench — symmetric to the pasadura↔toe correlation; (b) per-malla design achievement score (0–100%) with crest/toe/berm breakdown. Both consume existing fields; nothing existing changes.
...
- GIVEN the existing `tests/test_blast_correlation.py` corpus (pasadura + PF regression tests)
- WHEN `pytest tests/test_blast_correlation.py tests/test_blast_integration.py -v` runs after the change
- THEN every pre-existing test passes (no regression in PF or pasadura paths)

### Requirement: legacy public API surface preserved

Per `openspec/config.yaml` (`core/__init__.py` off-limits) and AGENTS.md import rules, the new symbols SHALL be imported from their submodule directly (`from core.blast_model import compute_stemming_crest_correlation`; `from core.blast_achievement import compute_design_achievement_score`). `core/__init__.py` SHALL NOT be modified.

#### Scenario: core/__init__.py untouched

- GIVEN the legacy public API re-exported by `core/__init__.py`
- WHEN `git diff main...HEAD -- core/__init__.py` runs after the change
...
- `core/__init__.py` re-exports stay intact. New symbols MAY be re-exported additively only (this change does not re-export them; they are imported from their submodules directly per AGENTS.md).
- `core/blast_model.py` gains one top-level function (`compute_stemming_crest_correlation`); `compute_pasadura_toe_correlation` and `compute_pf_damage_regression` signatures unchanged.
- `core/blast_achievement.py` is a new module; no existing module touched.
- `web/`, `api/`, `cli.py`, `app.py`, other `ui/*` files untouched.
- `ui/tabs/blast_correlation.py` touched under explicit scope override (blast tab only); see proposal §Scope override.
