# Capability: blast-hole-attribution

> **Source change**: `blast-hole-attribution` (archived)
> **Archived on**: 2026-07-12
> **Status**: archived, source of truth

> **Scope override**: temporarily lifts `openspec/config.yaml#off_limits` for `ui/tabs/blast_correlation.py` (blast tab only) per explicit maintainer authorization (one-off, not precedent — same pattern as `2026-07-12-blast-design-achievement`).

## Purpose

Define individual blast-hole attribution to measured crest and toe deviations without changing existing blast analysis. Engineers ask "which hole caused the overbreak on bench 5's crest?" — the system now attributes the top-N contributing holes per deviated feature, ranked by `kg / d²` (charge over squared horizontal distance), reusing the IDW scoring convention from `compute_energy_density_along_profile`.
