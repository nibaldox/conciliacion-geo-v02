...
class GeometryConfigurationSchema(BaseModel):
    ...
    geometry_configuration_version: str = "2.0"
...
class BlastUploadResponse(BaseModel):
    ...
    Remediación 3.2/4.2: the structured ``rejected_rows`` and
    ``blocking_errors`` survive even when zero rows are accepted — the
    endpoint returns HTTP 422 (not 400) in that case so the operator can
    read the structured diagnosis in the body.
    """
    ...
    # Auditoría §3.4 + remediación 4.5/4.6: structured warnings, rejections,
    # blocking errors and the versioned geometric configuration.
    data_warnings: str = ""
